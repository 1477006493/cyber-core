...
public class SensitiveFilter implements Serializable {
    ...
    private static final String SENSITIVE_WORDS = "sensi_words.txt";
    ...
    public static final SensitiveFilter DEFAULT = new SensitiveFilter(
            new BufferedReader(new InputStreamReader(
                    Objects.requireNonNull(ClassLoader.getSystemResourceAsStream(SENSITIVE_WORDS))
    ...
    public Boolean isSensitive(String sentence) {
        ...
        String filter = filter(sentence, "*");
        return !sentence.equals(filter);
    }

    /**
     * 对句子进行敏感词过滤
     * @param sentence 句子
     * @param replaceStr  敏感词的替换字符
     * @return 过滤后的句子
     */
    public String filter(String sentence, String replaceStr) {
        char[] replaces = replaceStr.toCharArray();
